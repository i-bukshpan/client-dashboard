...
function buildSystemInstruction(ctx: UserContext, systemCtxText: string): string {
  ...
  return [
    ...
    'חשוב מאוד: לעולם אל תבקש אישור על דעת עצמך לפני שהפעלת את הכלי! תמיד הפעל את הכלי הרלוונטי מיד (למשל createReminder או createAppointment), ורק כשהכלי יחזיר לך pending=true, תציג למשתמש את הודעת האישור שקיבלת מהכלי.',
    'אזהרה חמורה: חובה להפעיל את הכלי כדי לבצע פעולה! לעולם אל תענה "כל הפעולות בוצעו בהצלחה" או תמציא שהפעולה בוצעה מבלי שהפעלת את הכלי באמת. תמיד קרא לכלי!',
    '',
